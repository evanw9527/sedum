...
function EdgeInspector({ edge, nodes, onUpdateEdge, onRemoveEdge }) {
  ...
  return <aside className="v2-inspector edge-inspector"><header><div><h2>连线设置</h2><small>决定数据如何进入下一节点</small></div><button className="inspector-delete" type="button" aria-label="删除连线" onClick={() => onRemoveEdge(edge.id)}><Icon name="trash" size={16} /></button></header>
    ...
    <section className="edge-behavior-summary"><span className={condition ? 'is-conditional' : 'is-always'}><Icon name={condition ? 'condition' : 'success'} size={16} /></span><div><small>当前执行方式</small><strong>{condition ? '条件成立时通过' : '无条件直接通过'}</strong><p>{condition ? '来源节点的输出符合下方规则时，才会执行目标节点。' : '来源节点执行完成后，数据会直接进入目标节点。'}</p></div></section>
    <section className="edge-condition-editor"><div className="edge-condition-heading"><div><h3>执行条件</h3><p>只在需要做分支判断时添加。</p></div><select aria-label="执行方式" value={condition ? 'conditional' : 'always'} onChange={(event) => onUpdateEdge({ condition: event.target.value === 'always' ? null : { path: '', operator: 'truthy', value: null } })}><option value="always">无条件</option><option value="conditional">添加条件</option></select></div>
    ...
    <section className="edge-reconnect-help"><Icon name="edge" size={16} /><div><strong>调整连接</strong><p>拖动连线的起点或终点可改接节点；按 Delete 可删除当前连线。</p></div></section>
